...
def eval_preds(results: Optional[dict[str, dict[str, int]]]) -> Optional[pd.DataFrame]:
    ...
    with open("backend/today.json", "r") as f:
        games = json.load(f)
    date = list(results.keys())[0]
    if not games:
        print("No games on ", date)
        return None
    with open("backend/log.csv", "r") as f:
        log = pd.read_csv(f)
    ...
    log.to_csv("backend/log.csv", index=False)
    print(log)
    return df
...
def update_stats_table(df: Optional[pd.DataFrame]) -> Optional[list]:
    if df is None:
...
def create_df():
    df = pd.DataFrame(data=[], columns=["game_id", "home_team_id", "away_team_id", "prediction", "winner", "date"])
    df.to_csv("backend/log.csv", index=False)

if __name__ == "__main__":
    create_df() # Run this once to create the log.csv file
    yesterday = (datetime.now() - timedelta(1)).strftime('%m/%d/%Y')
    results = get_results("10/17/2024")
    print(results)
